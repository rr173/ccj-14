...
- **乐观并发**：提交前向服务端核对目标 / 来源分支权威 head。合并期间任一分支在另一页面已前进时返回 `409 merge-target-advanced`（或 `merge-source-advanced`），不追加事件，本地选择保留；在页面上“更新到最新分支头”后，旧草案标记 `superseded`，仍适用的旧选择按键保留，新出现 / 仍存在的冲突逐项重新确认即可；
